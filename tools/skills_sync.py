...
from hermes_constants import get_bundled_skills_dir, get_config_path, get_hermes_home
...
def _normalize_config_list(value) -> Set[str]:
    """Return a lower-cased set from a YAML scalar/list config value."""
    if value is None:
        return set()
...
def _read_sync_exclusions() -> Tuple[Set[str], Set[str]]:
    """Read optional skill-sync exclusions from config.yaml.

    These keys intentionally live outside the hub uninstall path: they let a
    user keep bundled categories/names tombstoned so future sync/update runs do
    not recreate them in ~/.hermes/skills/.
    """
    ...
    excluded_categories = _normalize_config_list(skills_cfg.get("excluded_categories"))
    excluded_categories |= _normalize_config_list(skills_cfg.get("sync_excluded_categories"))
    excluded_names = _normalize_config_list(skills_cfg.get("excluded_names"))
    excluded_names |= _normalize_config_list(skills_cfg.get("sync_excluded_names"))
    return excluded_categories, excluded_names


def _category_for_bundled_path(skill_dir: Path, bundled_dir: Path) -> str:
    """Return the top-level bundled category for a skill directory."""
    try:
...
def _is_sync_excluded(
    ...
    """Return True when config says this bundled skill must not be synced."""
    if skill_name.lower() in excluded_names:
        return True
    category = _category_for_bundled_path(skill_dir, bundled_dir)
    return bool(category and category in excluded_categories)


def _is_excluded_description(
    desc_md: Path,
    bundled_dir: Path,
    excluded_categories: Set[str],
) -> bool:
    """Return True when a category DESCRIPTION.md belongs to an excluded category."""
    try:
        rel = desc_md.relative_to(bundled_dir)
    except ValueError:
        return False
    return bool(rel.parts and rel.parts[0].lower() in excluded_categories)
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    excluded_categories, excluded_names = _read_sync_exclusions()
    bundled_skills = [
        (name, src)
        for name, src in all_bundled_skills
        if not _is_sync_excluded(name, src, bundled_dir, excluded_categories, excluded_names)
    ]
    # Keep excluded bundled skills in bundled_names so their manifest tombstones
    # are preserved instead of being treated as upstream-deleted entries.
    bundled_names = {name for name, _ in all_bundled_skills}

    copied = []
    updated = []
    user_modified = []
    skipped = len(all_bundled_skills) - len(bundled_skills)
    ...
    # except for categories explicitly tombstoned in config.
    for desc_md in bundled_dir.rglob("DESCRIPTION.md"):
        if _is_excluded_description(desc_md, bundled_dir, excluded_categories):
            continue
        rel = desc_md.relative_to(bundled_dir)
    ...
    return {
        ...
        "cleaned": cleaned,
        "total_bundled": len(all_bundled_skills),
    }
