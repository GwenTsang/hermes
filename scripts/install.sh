...
wait_for_apt_lock() {
    # Wait for apt/dpkg locks to be released (e.g. during Codespace/container init).
    # Without this, `apt-get install` (called by playwright --with-deps) blocks
    # indefinitely if another process holds the lock, and 2>/dev/null hides it.
    local max_wait=120
    local waited=0
    local lock_files="/var/lib/dpkg/lock-frontend /var/lib/apt/lists/lock /var/cache/apt/archives/lock"

    # Only relevant on apt-based systems
    ...
    while [ $waited -lt $max_wait ]; do
        ...
        if [ $waited -eq 0 ]; then
            log_info "Waiting for apt/dpkg lock to be released (another process is using it)..."
        fi
        sleep 2
        waited=$((waited + 2))
    done
    log_warn "apt/dpkg lock still held after ${max_wait}s — proceeding anyway (may fail)"
}
            ...
            case "$DISTRO" in
                ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                    ...
                    if [ "$(id -u)" -eq 0 ] || (command -v sudo >/dev/null 2>&1 && sudo -n true 2>/dev/null); then
                        ...
                        wait_for_apt_lock
                        cd "$INSTALL_DIR" && run_browser_install_with_timeout 600 npx playwright install --with-deps chromium || {
                        ...
                        cd "$INSTALL_DIR" && run_browser_install_with_timeout 600 npx playwright install chromium || {
                            log_warn "Playwright browser installation failed — browser tools will not work."
                ...
                *)
                    ...
                    cd "$INSTALL_DIR" && run_browser_install_with_timeout 600 npx playwright install chromium || true
                    ;;
            esac
        fi
        fi
