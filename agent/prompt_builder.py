...
def _coerce_bool_config(value, default: bool) -> bool:
    """Coerce permissive YAML/string config values to bool."""
...
def _skills_system_prompt_index_enabled(default: bool = True) -> bool:
    """Return whether the full skills index should be injected into the system prompt."""
    ...
    try:
        ...
        logger.debug("Could not read skills prompt index config %s: %s", config_path, e)
        return default
    ...
    if not isinstance(skills_cfg, dict):
        return default
    return _coerce_bool_config(skills_cfg.get("system_prompt_index", default), default)
...
def build_skills_system_prompt(
    ...
    include_full_index = _skills_system_prompt_index_enabled()
    cache_key = (
    ...
    if not skills_by_category:
        result = ""
    elif not include_full_index:
        result = (
            "## Skills\n"
            "Skills are available through skills_list, skill_view, and skill_manage, "
            "but the full skills index is intentionally omitted from the default system "
            "prompt to keep context compact. When a task clearly needs a reusable "
            "workflow, use skills_list if you need to discover available skills, then "
            "load the relevant skill with skill_view(name) before proceeding. "
            "For Hermes Agent configuration, setup, or troubleshooting, load the "
            "`hermes-agent` skill first. If a loaded skill is wrong or stale, patch it "
